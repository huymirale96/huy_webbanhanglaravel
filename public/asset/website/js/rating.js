        ...
        t.prototype.syncRating = function (e) {
            ...
            if (e) {
                for (t = n = 0, i = e - 1; 0 <= i ? n <= i : n >= i; t = 0 <= i ? ++n : --n) {
                    this.$el.find("i").eq(t).removeClass("far").addClass("fas")
                }
            }
            if (e && e < 5) {
                for (t = r = e; e <= 4 ? r <= 4 : r >= 4; t = e <= 4 ? ++r : --r) {
                    this.$el.find("i").eq(t).removeClass("fas").addClass("far")
                }
            }
            if (!e) {
...
$(function () {
    ...
    $('.btn-rating').click(function () {
        let comment = $('#product-review').val();
        var stars = ratingsField.val();
        ...
        $.ajax({
            ...
            success: function (res) {
                let new_comment = '<div class="comments-list"><div class="media">';
                new_comment += '<a class="media-left col-md-1" href="#"><img src="http://lorempixel.com/40/40/people/1/"></a>';
                new_comment += '<div class="media-body col-md-10"><h4 class="media-heading user_name col-md-8">';
                new_comment += res.customer.name;
                new_comment += ' <small>(' + res.comment.created_at + ')</small>';
                new_comment += '- ' + stars + ' <i class="fas fa-star"></i></h4>';
                new_comment += comment;
                new_comment += '</div></div></div><hr>';
                $('.new-comment').html(new_comment);
                $('#product-review').val('');
                $.toast({
                    heading: 'Thành công',
                    text: 'Đã đăng bình luận',
                    position: 'top-right',
                    icon: 'success'
                })
            },
            error: function () {
                alert('Đã xảy ra lỗi trong quá trình thực hiện, hãy thử lại sau');
            },
        });
    })
});
